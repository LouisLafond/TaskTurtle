...
function UserTasks() {
    ...
    // si aucune tache acceptee par l'utilisateur
    let taskNotCompleted = tasks.filter(task => task[3] === 0);
    if (taskNotCompleted.length == 0) {
        return (
            <div id="user-tasks">
                <Navigation />
                <h1 className='text-center'> { user.name }, Vous n'avez aucune tâche à faire, <a href="/tasks">aller en accepter une !</a></h1>
            </div>
        )
    } else {
        return (
